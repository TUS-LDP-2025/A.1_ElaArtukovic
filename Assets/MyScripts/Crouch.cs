...
public class Crouch : MonoBehaviour
{
    ...
    {
        RaycastHit hit;
        Vector3 rayOrigin = transform.position;     //add offset, might help detect collider if it overlaps with player collider before the crouch collider
        Debug.DrawRay(rayOrigin, Vector3.up * rayDistance, Color.purple);
        if (Physics.SphereCast(rayOrigin, 0.5f, Vector2.up, out hit, 5f, crouchableLayer, QueryTriggerInteraction.Ignore))
